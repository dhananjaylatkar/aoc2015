...
pub fn run() {
    let input_lines = lines_from_file("./src/input/day_02");
    part_1_2(&input_lines);
}

fn part_1_2(input: &Vec<String>) {
    let mut paper = 0;
    let mut ribbon = 0;

    for inp in input {
        let mut dimentions: Vec<i32> = inp
            .split("x")
            .map(|x| x.parse::<i32>().unwrap())
            .collect::<Vec<i32>>();
        dimentions.sort();
        paper += wrapping_paper(dimentions[0], dimentions[1], dimentions[2]);
        ribbon += ribbon_len(dimentions[0], dimentions[1], dimentions[2]);
    }

    println!("D02P1: {paper}");
    println!("D02P2: {ribbon}");
}
