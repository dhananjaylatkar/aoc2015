/*
  ...
  characters in strings. For example, C, JavaScript, Perl, Python, and even
  PHP handle special characters in very similar ways.
  ...
  on each line. The only escape sequences used are \\ (which represents a single
      ...
      plus two hexadecimal characters (which represents a single character with
          that ASCII code).
...
pub fn run() {
    let input_lines = lines_from_file("./src/input/day_08");
    part_1(&input_lines);
    part_2(&input_lines);
}
