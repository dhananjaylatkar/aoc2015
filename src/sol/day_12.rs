...
pub fn run() {
    let input_lines = lines_from_file("./src/input/day_12");
...
fn helper(v: &serde_json::Value, part: i32) -> i64 {
    let mut res = 0;
    if v.is_object() {
        let tmp = v.as_object().unwrap();
        if part == 2 && tmp.values().any(|val| val.is_string() && val == "red") {
            return 0;
        }
        for (_, val) in tmp {
            res += helper(&val, part);
