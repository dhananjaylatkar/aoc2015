...
delivers 1 * 10 = 10 presents. The fourth house gets 70 presents, because it is
...
with open("./src/input/day_20") as f:
    lines = f.readlines()
...
def part_1():
    ...
    print(f"D20P1: {np.argmax(presents >= n)}")
...
def part_2():
    ...
    for elf in range(1, n // 10):
        for house in range(elf, n // 10, elf):
            ...
            if house == 50 * elf:
                break
    print(f"D20P2: {np.argmax(presents >= n)}")
